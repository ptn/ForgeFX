...
// The registry normally resolves/opens the real serial/MIDI connection (transport/connection.ts).
// The mocked unit tests (test/drivers/*.test.ts, no hardware) build an ISOLATED registry with fake
// resolveConn/openConn via __createRegistryForTest() below. Production code must only ever use the
// `registry` singleton at the bottom of this file — never construct a DeviceRegistry itself.
interface RegistryDeps {
  resolveConn: typeof resolveConn;
  openConn: typeof openConn;
}
...
class DeviceRegistry {
  // Connection resolution/opening — the real transport/connection.ts functions in production, mock
  // implementations in the unit tests (see the TEST-ONLY seam above).
  #deps: RegistryDeps;
  constructor(deps: RegistryDeps = { resolveConn, openConn }) {
    this.#deps = deps;
  }
  ...
  async selectConnection(conn: Conn | null, model?: string | null) {
    ...
    return { ok: true, chosen: await this.#deps.resolveConn(), profileOverride: getProfileOverride() };
  }
...
// ── TEST-ONLY ──
/** Build an ISOLATED DeviceRegistry over mocked connection resolution/opening. For the mocked unit
 *  tests (test/drivers/*.test.ts) ONLY — the server always uses the `registry` singleton above. */
export function __createRegistryForTest(deps: RegistryDeps): DeviceRegistry {
