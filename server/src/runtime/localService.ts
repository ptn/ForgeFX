...
// supplies (directory-handle adapter, opaque root labels). Every method answers `{code, body}` so
...
/** One route result: HTTP status code + JSON body (Uint8Array for the raw .syx file read). */
export interface LocalResult { code: number; body: unknown }
...
export function createLocalService(deps: LocalServiceDeps) {
  ...
  const needRoot = async (): Promise<string | LocalResult> => {
    const c = cfg();
    if (!c.root) return { code: 409, body: { error: 'not configured' } };
    if (!(await adapterFor(c.root).exists(''))) return { code: 409, body: { error: 'root missing' } };
    return c.root;
  };
  let syncBusy = false;
  ...
  return {
    async config(): Promise<LocalResult> {
      return { code: 200, body: await configState() };
    },

    async setConfig(root: string | null | undefined): Promise<LocalResult> {
      const r = root ?? null;
      if (r === null) { saveCfg({ root: null, lastSync: null }); return { code: 200, body: await configState() }; }
      if (typeof r !== 'string' || !deps.isAbsolute(r)) return { code: 400, body: { error: 'absolute path required' } };
      const ad = adapterFor(r);
      try {
        await ad.mkdir(PRESETS_SUB);
        await ad.mkdir(SYNC_SUB);
      } catch (e) { return { code: 400, body: { error: `cannot create folders: ${(e as Error).message}` } }; }
      if (!(await writableProbe(ad))) return { code: 400, body: { error: 'folder is not writable' } };
    ...
    async presetFile(path: string | undefined): Promise<LocalResult> {
      ...
      if (rel == null || !isSyx(rel)) return { code: 400, body: { error: 'bad path' } };
      const ad = adapterFor(root);
      if (!(await ad.exists(`${PRESETS_SUB}/${rel}`))) return { code: 404, body: { error: 'not found' } };
      return { code: 200, body: await ad.readFile(`${PRESETS_SUB}/${rel}`) };
    },
    ...
    async writePreset(body: { name?: string; dir?: string; path?: string; bytes?: number[]; overwrite?: boolean } | undefined): Promise<LocalResult> {
      ...
      if (!Array.isArray(bytes) || !bytes.length) return { code: 400, body: { error: 'bytes[] required' } };
      let rel: string;
      if (path) {
        const r = safeRel(path);
        if (r == null || !isSyx(r)) return { code: 400, body: { error: 'bad path' } };
        rel = r;
      } else {
        const relDir = dir ? safeRel(dir) : '';
        if (relDir == null) return { code: 400, body: { error: 'bad dir' } };
        rel = `${relDir ? `${relDir}/` : ''}${sanitize(name ?? 'preset')}.syx`;
      }
      const ad = adapterFor(root);
      const file = `${PRESETS_SUB}/${rel}`;
      if ((await ad.exists(file)) && !overwrite) return { code: 409, body: { error: 'exists', path: rel } };
      try {
        await ad.mkdir(file.split('/').slice(0, -1).join('/'));
        await ad.writeFile(file, Uint8Array.from(bytes));
      } catch (e) { return { code: 500, body: { error: (e as Error).message } }; }
      return { code: 200, body: { ok: true, path: rel } };
    },
    ...
    async sync(): Promise<LocalResult> {
      const root = await needRoot();
      if (typeof root !== 'string') return root;
      if (syncBusy) return { code: 409, body: { error: 'busy' } };
      syncBusy = true;
      try {
    ...
    async restore(): Promise<LocalResult> {
      ...
      if (syncBusy) return { code: 409, body: { error: 'busy' } };
      syncBusy = true;
      try { return { code: 200, body: await restoreFromFolder(adapterFor(root), store, deps.sha256Hex) }; }
      catch (e) { return { code: 500, body: { error: (e as Error).message } }; }
      finally { syncBusy = false; }
    }
  };
