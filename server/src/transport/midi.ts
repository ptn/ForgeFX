...
export class MidiTransport implements Transport {
  ...
  async open(): Promise<void> {
    ...
    inp.on('message', (_dt, msg) => {
      if (msg[0] === SYSEX_START) {
        ...
        if (this.#tapPath) console.log(`[forgefx][midi] RX ${frame.length}B: ${frame.slice(0, 64).map((b) => b.toString(16).padStart(2, '0')).join(' ')}${frame.length > 64 ? '…' : ''}`);
  ...
  send(bytes: readonly number[]): void {
    ...
    if (this.#tapPath) console.log(`[forgefx][midi] TX ${bytes.length}B: ${bytes.slice(0, 64).map((b) => b.toString(16).padStart(2, '0')).join(' ')}${bytes.length > 64 ? '…' : ''}`);
    this.#out.send([...bytes]);
  }
